...
module.exports = (client, commandOptions) => {
  ...
  client.on('message', async (message) => {
    ...
    const prefix = (guild) ? (guildPrefixes[guild.id] || globalPrefix) : globalPrefix
      ...
      if (
        ...
        for (const requiredRole of requiredRoles) {
          ...
          if (!role || !member.roles.cache.has(role.id)) {
            message.reply(
              `To use this command, you must have the "**${requiredRole}**" role.`
            )
            return
          }
        }
        ...
        if (cooldown > 0 && recentlyRan.includes(cooldownString)) {
          message.reply('There is a cooldown for this command, please wait until it is over.');
          return
        }
